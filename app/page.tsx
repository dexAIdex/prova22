...
export default function Home() {
  ...
  const calculateTokensToBuy = async (
    ...
    try {
      ...
      const bnbUsdPriceOnChain = Number(priceFromOracle) / 1e8; // Usually 8 decimals in Chainlink BNB/USD

      const pricePerToken = 0.0001 + (Number(currentStage) - 1) * 0.000025;

      const tokensToBuy =
        (Number(bnbAmountInput) * bnbUsdPriceOnChain) / pricePerToken;

      console.log("TokensToBuy (calculated from contract data):", tokensToBuy);
  ...
  const handleBuyWithBNB = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log("Non esiste tokensSold() nel contratto?");
      }
